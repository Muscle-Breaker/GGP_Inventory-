...
export default function TransactionsPage() {
  ...
  const handleSave = async () => {
    if (!form.product_id || !form.quantity) { setError('상품과 수량을 입력해주세요'); return; }
  ...
  return (
    <div className="p-6 space-y-5 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-wrap gap-2">
        {TX_TYPES.map(t => (
          <button key={t.value} onClick={() => openModal(t.value)}
            ...
            {(t.value === 'STOCK_IN' || t.value === 'RETURN') ? <ArrowDown size={14} /> : <ArrowUp size={14} />}
            {t.label} 등록
          </button>
        ))}
      </div>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <select value={form.product_id} onChange={e => setForm(f => ({ ...f, product_id: e.target.value }))}
                    ...
                    {products.map(p => (
                      <option key={p.id} value={p.id}>{p.name} ({p.sku}) — 현재재고: {p.current_stock}</option>
                    ))}
                  </select>
                  <ChevronDown size={14} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={form.quantity} onChange={e => setForm(f => ({ ...f, quantity: e.target.value }))}
                    className="form-input" placeholder="0" min="1" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <select value={form.sales_channel} onChange={e => setForm(f => ({ ...f, sales_channel: e.target.value }))}
                      ...
                      {channels.filter(c => c.is_active).map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                    </select>
                    <ChevronDown size={14} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                  </div>
                </div>
              ...
              <div>
                <label className="form-label">{form.type === 'OTHER_OUT' ? '출고 사유 *' : '비고'}</label>
                <input type="text" value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))}
                  className="form-input" placeholder={form.type === 'OTHER_OUT' ? '출고 사유를 입력하세요' : '메모 (선택사항)'} />
              </div>
