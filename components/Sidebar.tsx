...
import {
  ...
  BarChart3, Settings, Menu, X, ShoppingBag
} from 'lucide-react';
...
const navItems = [
  ...
  { href: '/inventory',    label: '재고 관리',   icon: Layers },
  { href: '/transactions', label: '입출고 관리', icon: ArrowLeftRight },
    ...
    <>
      ...
      <div className="px-4 py-3 border-t border-gray-100">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 bg-gray-200 rounded-full flex items-center justify-center text-xs font-bold text-gray-600">관</div>
          <div>
    ...
    <>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-30" onClick={() => setMobileOpen(false)}>
          ...
          <aside className="absolute top-0 left-0 bottom-0 w-56 bg-white flex flex-col shadow-xl" onClick={e => e.stopPropagation()}>
            <NavLinks />
          </aside>
        </div>
      )}
